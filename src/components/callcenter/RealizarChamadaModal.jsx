...
// modoMicroSIP=false → usa API NVOIP; modoMicroSIP=true → abre protocolo microsip:
export default function RealizarChamadaModal({ open, onOpenChange, numeroInicial = '', onChamadaIniciada, modoMicroSIP = false }) {
  ...
  const handleLigar = async () => {
    ...
    if (modoMicroSIP) {
      // Modo MicroSIP: abre protocolo microsip:
      const a = document.createElement('a');
      a.href = `microsip:${numero}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      toast.success('Discando via MicroSIP...');
    ...
    // Modo NVOIP API: chama o backend
    setLigando(true);
    try {
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
            ...
            <p className="mt-1">
              {modoMicroSIP
                ? 'O MicroSIP discará diretamente para o número informado.'
                : 'O sistema NVOIP ligará para o número via API. Atenda o retorno no seu chip configurado.'}
            </p>
          </div>
