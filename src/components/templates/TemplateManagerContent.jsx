...
/**
 * Conteúdo do gerenciador de templates da API Oficial Meta.
 * Reutilizado tanto dentro do Sheet (TemplateManagerModal) quanto de forma
 * embedded na aba "Templates" do módulo Campanhas.
 */
export default function TemplateManagerContent({
  ...
  const mostrarErroAprovacao = (errData) => {
    if (mostrarDiagnosticoConexao(errData)) return;
  ...
  // Carrega dados ao montar e sincroniza com a Meta em background
  useEffect(() => {
    let mounted = true;
    loadConnections();
    loadTemplates();
    syncTemplatesFromMeta()
      .then(() => { if (mounted) loadTemplates(); })
      .catch(() => {});
    ...
    return (
      <div className="text-center py-10 px-6 text-sm text-slate-500">
        ...
        <div className="mt-3 text-xs">
          Templates aprovados poderão ser utilizados em campanhas e no bate-papo quando disponíveis.
        </div>
        {onRequestClose && (
          <Button className="mt-4" variant="outline" onClick={onRequestClose}>Fechar</Button>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="px-5 py-3 border-b border-slate-200 flex items-center justify-between flex-wrap gap-2 bg-white">
        <div className="text-base font-bold text-slate-800">Gerenciar Templates da API Oficial</div>
      ...
      {tab === 'criar' && (
        <div className="border-t border-slate-200 px-5 py-3 flex items-center justify-end gap-2 bg-white">
          <Button variant="outline" onClick={handleCancelar} disabled={isLoading}>Cancelar</Button>
          <Button
            variant="outline"
          ...
          >
            {isSending ? (
              <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Enviando para aprovação...</>
            ) : (
