...
export default function ComissoesPagar() {
  ...
  const gerarPDF = (comissoesLista, vendedorInfo, dataPagamento, formaPagto, loteCode) => {
    ...
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text('Vendedor:', 14, 33);
    doc.text('Data Pagamento:', 90, 33);
    doc.text('Forma Pagamento:', 160, 33);
    doc.text('Qtd. Itens:', 230, 33);
    doc.setFont('helvetica', 'normal');
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('Total Recebido (Adm):', 14, 48);
    doc.text('Total Pago ao Vendedor:', 120, 48);
    doc.setTextColor(0, 120, 80);
    doc.text(fmt(totalRecebido), 70, 48);
    doc.setTextColor(0, 80, 180);
    doc.text(fmt(totalPago), 200, 48);
    ...
    doc.autoTable({
      ...
      columnStyles: {
        4: { halign: 'right' },
        6: { halign: 'right', textColor: [0, 80, 180] },
      },
    });
    ...
    doc.text(`Documento gerado por ${user?.full_name || 'Sistema'} em ${moment().format('DD/MM/YYYY HH:mm')}`, 148, pageHeight - 5, { align: 'center' });

    doc.save(`comissao_${vendedorInfo?.vendedor_nome?.replace(/\s+/g, '_') || 'vendedor'}_${moment(dataPagamento).format('YYYYMMDD')}.pdf`);
  };

  const handlePagarEGerarPDF = async () => {
    if (itensSelecionados.size === 0 || !vendedorPagamento) return;
    setIsPaying(true);

    try {
      const comissoesIds = Array.from(itensSelecionados);
      const comissoesParaPagar = comissoes.filter(c => comissoesIds.includes(c.id) && c.status_pagamento === 'a_pagar');

      if (comissoesParaPagar.length === 0) {
        toast.error('Nenhuma comissão válida para pagar');
        return;
      }
      ...
      const totalPago = comissoesParaPagar.reduce((acc, c) => acc + (c.valor_a_pagar || 0), 0);
      ...
      await base44.entities.PagamentoComissaoLote.create({
        ...
        data_pagamento: dataPagamento,
        forma_pagamento: formaPagamento,
        total_itens: comissoesParaPagar.length,
        total_pago: totalPago,
        observacao,
        gerado_por_id: user.colaborador_id,
        gerado_por_nome: user.full_name,
        comissoes_ids: JSON.stringify(comissoesParaPagar.map(c => c.id)),
        email_enviado: false,
      });

      // Gerar PDF
      gerarPDF(comissoesParaPagar, vendedorPagamento, dataPagamento, formaPagamento, loteCode);

      queryClient.invalidateQueries(['comissoes-a-pagar']);
      toast.success(`✅ ${comissoesParaPagar.length} comissão(ões) paga(s)! PDF gerado.`);
