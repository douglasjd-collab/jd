...
import {
  ...
  FileText, Loader2, CheckCircle2, Clock, DollarSign,
  Eye, BarChart2, Calendar
} from 'lucide-react';
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-900">
          Comissões a Pagar
          {user?.empresa_nome && <span className="text-slate-400 font-normal ml-2">• {user.empresa_nome}</span>}
        </h1>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {vendedoresComComissoes.map((vendedor) => {
            const totalAPagar = vendedor.comissoes.filter(c => c.status_pagamento === 'a_pagar').reduce((a, c) => a + (c.valor_a_pagar || 0), 0);
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-white/70 mt-0.5">
                      ...
                      {ultimoRec && (
                        <>
                          <span>•</span>
                          <span>Último recebimento: {formatDateBR(ultimoRec)}</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
                    ...
                    {qtdAPagar > 0 && isAdmin && (
                      <Button
                        size="sm"
                        className="bg-[#23BE84] hover:bg-[#1da872] text-white border-0"
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b bg-slate-50 text-slate-600">
                          {isAdmin && <th className="p-3 w-10 text-left"><Checkbox /></th>}
                              ...
                              {isAdmin && (
                                <td className="p-3">
                                  {isPagar ? (
                                    <Checkbox
                                      checked={false}
                                      onCheckedChange={() => abrirModalPagamento(vendedor, null)}
                                    />
                                  ) : <div className="w-4" />}
                                </td>
                              )}
      ...
      <Dialog open={pagarModal} onOpenChange={setPagarModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <div className="relative mb-2">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <Input
              placeholder="Buscar vendedor, grupo ou cota..."
              value={modalSearch}
                ...
                {comissoesModal.map(c => {
                  ...
                  return (
                    <tr key={c.id} className={`border-b transition-colors ${isSel ? 'bg-blue-50' : 'hover:bg-slate-50'}`}>
                      ...
                      <td className="p-3 text-right">
                        {isPagar ? (
                          <Badge className="bg-orange-100 text-orange-700 font-semibold">{fmt(c.valor_a_pagar)}</Badge>
                        ) : (
                          <Badge className="bg-green-100 text-green-700 font-semibold">{fmt(c.valor_a_pagar)}</Badge>
                        )}
                      </td>
                    </tr>
                  );
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setPagarModal(false)} disabled={isPaying}>
              Cancelar
            </Button>
            <Button
              disabled={modalSelecionados.size === 0 || isPaying}
