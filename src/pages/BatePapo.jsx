...
export default function BatePapo() {
  ...
  useEffect(() => {
    ...
    if (!conversaSelecionada) {
      ...
      if (conversaAtualizada) {
        ...
        setConversaSelecionada(prev => prev ? { ...prev, status: conversaAtualizada.status, ultima_mensagem: conversaAtualizada.ultima_mensagem, data_ultima_mensagem: conversaAtualizada.data_ultima_mensagem, ultimo_remetente: conversaAtualizada.ultimo_remetente, tipo_conexao: conversaAtualizada.tipo_conexao, instancia: conversaAtualizada.instancia, phone_number_id_meta: conversaAtualizada.phone_number_id_meta, canal_atendimento: conversaAtualizada.canal_atendimento, canal_preferencial: conversaAtualizada.canal_preferencial, canal_origem: conversaAtualizada.canal_origem, provider: conversaAtualizada.provider, locked_provider: conversaAtualizada.locked_provider, last_inbound_provider: conversaAtualizada.last_inbound_provider, responsavel_id: conversaAtualizada.responsavel_id, responsavel_nome: conversaAtualizada.responsavel_nome, responsavel_expira_em: conversaAtualizada.responsavel_expira_em, atendimento_prioritario: conversaAtualizada.atendimento_prioritario, bloqueado: conversaAtualizada.bloqueado, foto_url: conversaAtualizada.foto_url } : prev);
      } else {
  ...
  const enviarMensagemMutation = useMutation({
    mutationFn: async ({ texto, arquivo, mensagemParaResponder }) => {
      ...
      const resp = await base44.functions.invoke('enviarMensagemWhatsapp', {
        ...
        resposta_para_message_id: mensagemParaResponder?.whatsapp_message_id || null, resposta_para_id: mensagemParaResponder?.id || null,
      });
  ...
  // Conversa está em atendimento: atendente respondeu nos últimos 15 min OU sem remetente definido
  ...
  const contadores = {
    ...
    encerrada: conversas.filter(ehFinalizada).length,
  ...
  const conversasFiltradas = conversas
    .filter(c => {
      ...
      // Grupos: aparecem em 'grupos' e 'todas'
      if (isGrupo(c)) {
        const ativo = c.bloqueado !== true && c.bloqueado !== 'true';
        return ativo && (filtroStatus === 'grupos' || filtroStatus === 'todas');
      }
                          ...
                          <div className="jd-chat-content">
                            ...
                            {(() => {
                              ...
                              if (c.status === 'encerrada') return (
                                <span className="inline-flex w-fit max-w-full self-start items-center gap-1 px-1.5 py-0.5 rounded-full mb-0.5" style={{ background: '#f1f5f9', fontSize: '10px', color: '#64748b', fontWeight: 500 }}>
                              ...
                              if (atendenteAtivo) return (
                                <span className="inline-flex w-fit max-w-full self-start items-center gap-1 px-1.5 py-0.5 rounded-full mb-0.5" style={{ background: '#dcfce7', fontSize: '10px' }}>
                            ...
                            {microtarefasPorConversa[c.id]?.[0] && (() => {
                              ...
                              return (
                                <div className={`mb-1 inline-flex w-fit max-w-full self-start items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${atrasada ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-700'}`}>
