...
async function processMessageReceived(base44, body, connection, empresaId) {
  ...
  // Interceptar reações ANTES do fluxo fromMe. Quando a reação é feita pelo
  // celular conectado, ela chega como fromMe=true, mas continua sendo apenas
  // uma atualização da mensagem original — nunca uma nova mensagem.
  const reactionResult = await processReactionDapi(base44, data, empresaId);
...
async function processMessageSentFromPhone(base44, data, connection, empresaId, externalMessageId) {
  ...
  await base44.asServiceRole.entities.ConversaWhatsapp.update(conversa.id, {
    ...
    responsavel_expira_em: conversa.responsavel_expira_em
